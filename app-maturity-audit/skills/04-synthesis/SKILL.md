...
Verdict (the four signals, including the TCO signature), a scorecard reference, key findings
per dimension (weakest first, each linking to that dimension's `detail.md#findings`), the
prioritized roadmap (quick-wins → next → strategic), and methodology & limitations (static
audit; performance inferred; TCO signature is relative, not monetary). Include a short **TCO &
System Profile** note: the recorded composition from `recon.md` and the cost-of-ownership
reading, with adaptability's resistance hotspots called out as the dominant cost driver.
...
doesn't belong here. Include the **cost-of-ownership** reading anchored on the System Profile,
and translate **adaptability** into business terms: estimate reliability and one-directional
overrun risk (e.g. "roadmap items touching [hotspot] will tend to run over their estimates,
predictably in one direction; discount budget/timeline confidence there") — never "the system
is stubborn." Keep the TCO claim relative and evidenced; do not invent monetary figures.
