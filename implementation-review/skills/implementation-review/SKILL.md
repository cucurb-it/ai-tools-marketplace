...
### Step 5 — Output gh CLI commands (primary)

Generate ready-to-run `gh` CLI commands for all selected issues:

```bash
# Must Fix — Bug issues
gh issue create \
  --repo "{{GITHUB_REPO}}" \
  --title "[CRITICAL] Missing null check in UserService.GetById" \
  --body "$(cat <<'EOF'
## Review Finding
...
EOF
...
# Should Fix — Task issues
gh issue create \
  --repo "{{GITHUB_REPO}}" \
  --title "[HIGH] Missing input validation on CreateOrder endpoint" \
  --body "$(cat <<'EOF'
## Review Finding
...
EOF
)" \
  --label "task,review-finding{{,USER_LABEL}}"
```

Output all commands in a **single fenced code block** so the user can copy and run them in sequence.

### Step 6 — Output markdown fallback (secondary)

After the CLI commands, also provide a **collapsible markdown summary** the user can paste into GitHub manually if they prefer not to use the CLI:

```markdown
<details>
<summary>GitHub Issues — Manual Creation</summary>

### Issue 1: [CRITICAL] Missing null check in UserService.GetById
...
### Issue 2: [HIGH] Missing input validation on CreateOrder endpoint
...
- If the user has not specified a repository, ask before generating any commands
...
- **Creates GitHub Issues** for Must Fix and Should Fix findings (optional, via gh CLI or markdown)
